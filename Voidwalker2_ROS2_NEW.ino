...
//// Timers Setup ////
MyTimer blynkTimer_1s;
MyTimer blynkTimer_5s;
MyTimer GNSS_timer;
...
//SETUP FOR VOIDWALKER2 
    ...
    //timersettings
    blynkTimer_1s.start(1000);
    blynkTimer_5s.start(5000);
    GNSS_timer.start(200);
...
void loop()
{
    ...
    //VOID LOOP OF VOIDWALKER2
    // Timer 1: Execute every timer1Interval milliseconds
    if (blynkTimer_1s.isReady()) {
        blynkTimer_1s.restart();
        updateBlynk1s();
    }

    // Timer 2: Execute every timer2Interval milliseconds
    if (blynkTimer_5s.isReady()) {
        blynkTimer_5s.restart();
        updateBlynk5s();
    }

    // Check GNSS device periodically
    if (GNSS_timer.isReady()) {
        GNSS_timer.restart();
